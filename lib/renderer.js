...
class Renderer {
	...
	/**
	 * Create an Express view engine function.
	 * See the {@link Renderer#render} method for more information.
	 *
	 * @access public
	 * @returns {Function}
	 *     Returns an Express view engine function.
	 */
	express() {
		return async (filePath, renderContext, done) => {
			try {
				return done(null, await this.render(filePath, renderContext));
			} catch (error) {
				return done(error);
			}
		};
